...
if ( ! function_exists( 'adswth_header_cart' ) ) {
	...
	function adswth_header_cart() {
		...
		?>
		<a href="<?php echo $link ?>" class="btn-cart <?php echo $class; ?>">
            <div class="btn-cart-inner header-cart">
                <i class="icon-<?php echo adswth_option( 'header_cart_icon' )?>" style="color: <?php echo adswth_option( 'header_cart_icon_color' ); ?>"></i>
                <?php if( $count > 0 ){ ?>
                    <span class="cart-count icon-cart-count"><?php echo $count; ?></span>
                <?php } //endif; ?>
            </div>
		</a>
	<?php }
...
function adswth_woocommerce_btn_cart_count( $fragments ) {
	...
	$fragments['.icon-cart-count'] = ob_get_clean();
	return $fragments;
}
...
function adswth_popup_cart(){
    ...
    if( !is_admin() && !is_cart() && !is_checkout() && !is_checkout_pay_page() ) {
        $openAfterAdd = '';
        if (adswth_option("show_side_shoppingcart_after_product_add")){
            $openAfterAdd = 'openAfterAdd';
        }
        ?>

    <div id="cart-sidebar" class="cart-sidenav <?php echo $openAfterAdd ?>">
        <div class="cart-header">
            <h2><?php _e('Shopping cart', 'elgrecowoo');?></h2>
